...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      "fixed top-4 right-4 z-[100] flex max-h-screen w-[calc(100vw-2rem)] max-w-[420px] flex-col gap-2",
      className,
    )}
    {...props}
  />
...
>(({ className, variant, ...props }, ref) => {
  ...
  return (
    <ToastPrimitives.Root ref={ref} className={cn(toastVariants({ variant }), className)} asChild {...props}>
      <motion.div
        initial={prefersReducedMotion ? { opacity: 0 } : { opacity: 0, x: 16, scale: 0.95 }}
